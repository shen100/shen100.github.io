...
async function requestList() {
    let url = `/api/fictions?page=${page.value}`;
    try {
...
function onCreate() {
    router.push({ path: '/fictions/edit' })
}

function onEditChapter(id) {
    router.push({ path: `/fictions/${id}/chapters` })
}

function onEdit(id) {
    router.push({ path: '/fictions/edit', query: { id } })
}
...
async function deleteBook() {
    let url = `/api/fictions/${curId.value}`;
    try {
    ...
    <div>
        <Card>
            ...
            <Table :columns="columns" :data="list" border>
                ...
                <template #action="{ row }">
                    <Button @click="onEditChapter(row._id)" type="primary" size="small" style="margin-right: 10px;">编辑章节</Button>
