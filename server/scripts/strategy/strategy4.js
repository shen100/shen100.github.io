...
export function findHighRateInRecentNDays(allItems, stockDetail, options) {
    ...
    for (let i = 0; i < items.length; i++) {
        if (items[i].closePrice > highPrice) {
            ...
            highDate = items[i].date;
        }
        if (items[i].lowPrice < lowPrice) {
            lowPrice = items[i].lowPrice;
            lowDate = items[i].date;
        }
    }
...
export function detectTrend(allItems, stockDetail, options) {
    ...
    let rightIndex = -20;
    if (!findHighRateInRecentNDays(allItems, stockDetail, { rightIndex } ).ok) {
        return { ok: false };
    }
    let items = allItems.slice(rightIndex);
    let highPrice = 0;
